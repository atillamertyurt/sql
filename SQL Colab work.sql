...
--The naep table contains each state’s average NAEP scores in math and reading for students in grades 4 and 8 for various years 
...
--Data exploration
--You'll begin your analysis with the naep table. It's always a good idea to get a better understanding of your data 
--BEFORE doing any analysis. 
--This allows you to gather key insights before you jump into any complex operations. 
--You'll want to know what columns are reported in your data, what the data types are for each column, and what 
--the first few observations look like.
...
--5-Write a query that returns a field called bottom_10_states that lists the states in the bottom 10 for avg_math_4_score in 
--the year 2000.
...
--7- Write a query that returns a field called below_average_states_y2000 that lists all states with an avg_math_4_score less than
...
--8- Write a query that returns a field called scores_missing_y2000 that lists any states with missing values in the avg_math_4_score
...
--9- Write a query that returns for the year 2000 the state, avg_math_4_score, and total_expenditure from 
--the naep table left outer joined with the finance table, using id as the key and ordered by total_expenditure greatest to least. 
--Be sure to round avg_math_4_score to the nearest 2 decimal places, and then filter out NULL avg_math_4_scores in order to see 
--any correlation more clearly.
